...
| Kế thừa & bảo tồn chi phí riêng | Bảo tồn dòng nhập tay dưới chi phí chung, giữ số tiền trong FY và xóa số tiền khi sang FY mới | `test_manual_special_cost_sections.py` | Được bảo vệ |
| Sắp xếp thứ tự dòng chi phí | Metadata sheet ẩn `_mp2027_manual_special_meta`, layout merge, bảo toàn công thức và thứ tự kéo-thả | `test_output_cost_row_ordering.py` | Được bảo vệ |
| Phân tích biến động cùng kỳ (YoY) & Biểu đồ | Giải công thức AST `_MpFormulaResolver`, trích xuất Top 12 biến động, fallback font CJK/VN và nhúng biểu đồ native Excel | `test_variance_analyzer.py`, `test_variance_chart.py` | Được bảo vệ |
| An toàn giao diện & Singleton Editor | Chống mở trùng 6 cửa sổ công cụ, duy trì layering và khóa pipeline busy khi export | `test_singleton_editor_windows.py`, `test_variance_window_layering.py` | Được bảo vệ |
