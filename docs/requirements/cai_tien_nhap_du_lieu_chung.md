...
6. **Cụm “6 chi phí / gộp thành 1 dòng chi phí” chưa đủ căn cứ để tự động hóa ngay**
   File có ghi ở sheet `Hạng mục cần cải tiến`: “6 chi phí” và “gộp thành 1 dòng chi phí”. Tuy nhiên từ text cào ra chưa đủ chắc để kết luận nó áp dụng cho nhóm nào. Agent **không được tự ý gộp Facility 6 khoản** nếu chưa đối chiếu ảnh/FORM/repo hoặc chưa được xác nhận.
...
### 6.6. “6 chi phí / gộp thành 1 dòng chi phí” — điểm cần cẩn trọng
...
Chưa đủ căn cứ từ text để kết luận chính xác nhóm nào phải gộp.

Yêu cầu an toàn:

- Với `Chi phí hệ thống`: đã có yêu cầu rõ “Nhập vào 1 dòng duy nhất” → phải gộp các chi tiết hệ thống vào 1 dòng.
- Với `Chi phí khấu hao, lãi nhà đất, điện nước`: file ghi thứ tự 1~6 cho 6 khoản → **không tự động gộp 6 khoản này** nếu chưa được xác nhận.
- Với các khoản hành chính/event: mỗi content phải theo rule account/tháng riêng → không tự gộp nếu account/tháng khác nhau.
- Nếu agent thấy repo hiện tại đang có logic “gộp 6 chi phí”, phải tạo GAP và hỏi/xác nhận trước khi sửa mạnh.
...
- Nhưng audit vẫn phải lưu detail từng thành phần.
...
### 8.6. Điểm chưa được phép tự suy luận

Cụm “6 chi phí / gộp thành 1 dòng chi phí” chưa đủ chắc để áp dụng cho Facility.
Mặc định an toàn:

```text
Facility có 6 khoản theo thứ tự 1~6; không gộp thành 1 dòng nếu account/FORM row khác nhau.
```

Nếu repo hiện tại đang gộp, phải đưa vào GAP review.
...
3. **“6 chi phí / gộp thành 1 dòng chi phí”**
   Chưa đủ căn cứ để áp dụng cho Facility. Chỉ chắc chắn `Chi phí hệ thống` phải nhập 1 dòng duy nhất.
...
Sau khi lập GAP, đề xuất kế hoạch sửa nhỏ theo phase, mỗi phase có test gate rõ ràng. Không tự ý gộp 6 chi phí facility nếu chưa xác nhận. Không nhầm mã 1412... là account code.
```
