...
| Fixed assets calculation/writer | Fixed Assets module | OPEN_DEEP_AUDIT; NOT_ACCEPTED_FIXED_ASSETS_CALCULATION; IMPLEMENTATION_PARTIAL_PENDING_BUSINESS_DECISIONS | dynamic category rows; historical staging rows 38/42 | `docs/audits/fixed_assets_true_mismatch_decision_matrix_2026-07-16.md` completes provenance/decision classification for 638/638 true mismatches. `fixed_assets_policy_output_verification_2026-07-16.json` proves the production parser/writer matches 649 FY2026 and 726 FY2027 source-derived monthly cells with zero post-terminal facts. Per-asset rounding, Q/cache fail-closed, zero-vs-blank, source-row identity, DB provenance and FY-scoped imports are implemented/tested; do not treat 361 manual/other-layer rows, 200 snapshot contradictions or 73 source-indeterminate rows as automatic source data. |
