...
def _validate_stage_evidence(path: Path, run_id: str) -> dict[str, Any]:
    ...
    if payload.get("schema_version") != 1:
        raise ValueError(f"Unsupported stage evidence schema: {payload.get('schema_version')!r}")
    if str(payload.get("run_id") or "") != str(run_id):
        raise ValueError(f"Stage evidence run id mismatch: {payload.get('run_id')!r}")
    if payload.get("status") != "SUCCEEDED":
        raise ValueError(f"Stage evidence is not terminal SUCCEEDED: {payload.get('status')!r}")
    if payload.get("current_stage") not in (None, ""):
        raise ValueError(f"Stage evidence still has an active stage: {payload.get('current_stage')!r}")
    stages = payload.get("stages")
    if not isinstance(stages, list):
        raise ValueError("Stage evidence stages must be a list")
    ...
    if names != list(REQUIRED_PIPELINE_STAGES) or len(stages) != len(names):
        raise ValueError(
            f"Stage evidence sequence mismatch: expected={list(REQUIRED_PIPELINE_STAGES)}, actual={names}"
        )
    for stage in stages:
        if stage.get("status") != "PASS":
            raise ValueError(f"Pipeline stage is not PASS: {stage.get('name')!r}")
        elapsed = stage.get("elapsed_seconds")
        if not isinstance(elapsed, (int, float)) or isinstance(elapsed, bool) or elapsed < 0:
            raise ValueError(f"Pipeline stage has invalid elapsed time: {stage.get('name')!r}")
    total = payload.get("total_elapsed_seconds")
    if not isinstance(total, (int, float)) or isinstance(total, bool) or total < 0:
        raise ValueError("Stage evidence has invalid total elapsed time")
    return {
        "path": str(path),
...
def _catalog_row(catalog: Path, run_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        if row is None:
            raise ValueError(f"Run id is absent from the catalogue: {run_id}")
        return dict(row)
    finally:
        connection.close()
...
def _validate_staffing(
    ...
    *,
    ...
    try:
        ...
        if missing:
            raise ValueError("Run database is missing tables: " + ", ".join(missing))
        ...
        if len(baseline_rows) != 1:
            raise ValueError(
                f"Expected one manual baseline for {target_cc}/{baseline}; found {len(baseline_rows)}"
            )
        baseline_row = baseline_rows[0]
        if str(baseline_row["description"] or "") != expected_provenance:
            raise ValueError(
                f"Manual baseline provenance is not approved: {baseline_row['description']!r}"
            )
        ...
        if len(override) != 1 or str(override[0]["description"] or "") != expected_provenance:
            raise ValueError("Approved baseline override is absent from run.db")
        ...
        def assert_balanced(row: sqlite3.Row, label: str) -> None:
            ...
            if any(value is None for value in fields):
                raise ValueError(f"Staffing row has missing components: {label}")
            total, expat, staff, worker, local_total = map(float, fields)
            if any(value < 0 for value in fields):
                raise ValueError(f"Staffing row has negative components: {label}")
            if abs(local_total - staff - worker) > 1e-6:
                raise ValueError(f"Local staffing is not balanced: {label}")
            if abs(total - expat - local_total) > 1e-6:
                raise ValueError(f"Total staffing is not balanced: {label}")
        ...
        if missing_periods or duplicate_periods:
            raise ValueError(
                f"Department staffing coverage invalid; missing={missing_periods}, non_unique={duplicate_periods}"
            )
        ...
        if all_inputs <= 0 or target_inputs <= 0:
            raise ValueError(f"No real input facts: all={all_inputs}, target={target_inputs}")
        return {
        ...
        for row in hub.iter_rows(min_row=26, min_col=6, max_col=17):
            for cell in row:
                ...
                if isinstance(cell.value, str) and cell.value.startswith("="):
                    formulas += 1
        if populated == 0:
            raise ValueError("Output hub contains no populated business-month cells")
    ...
    *,
    ...
    try:
        ...
        if int(row["fiscal_year"]) != fiscal_year:
            raise ValueError(f"Catalogue fiscal year mismatch: {row['fiscal_year']}")
        if str(row["selected_cost_center"] or "") != target_cc:
            raise ValueError(f"Catalogue target CC mismatch: {row['selected_cost_center']!r}")
        if str(row["status"] or "") != "SUCCEEDED":
            raise ValueError(f"Catalogue status is not SUCCEEDED: {row['status']!r}")
        if abs(float(row["exchange_rate"]) - float(expected_exchange_rate)) > 1e-6:
            raise ValueError(f"Catalogue exchange rate mismatch: {row['exchange_rate']!r}")
        if row["error_summary"] not in (None, ""):
            raise ValueError(f"Successful run has an error summary: {row['error_summary']!r}")
    ...
    try:
        ...
        if not isinstance(sources, dict) or not sources:
            mismatches["source_checksums"] = "non-empty mapping required"
        if not manifest.get("template_checksum") or not manifest.get("manual_input_checksum"):
            mismatches["checksums"] = "template and manual-input checksums required"
        if mismatches:
    ...
    try:
        ...
        if preflight.get("ok") is not True or preflight.get("issues"):
            raise ValueError("Preflight report is not clean")
        if int(preflight.get("fiscal_year")) != fiscal_year:
            raise ValueError("Preflight fiscal year mismatch")
    ...
    if failure_traceback.exists():
        result.fail("failure_evidence", f"Successful workspace contains {failure_traceback}")
    else:
    ...
    try:
        ...
        if staged_hash is None or public_hash != staged_hash:
            raise ValueError("Published target workbook is not byte-identical to run workspace")
        preserved: dict[str, str] = {}
        for filename, expected_hash in (preserved_public_workbooks or {}).items():
            actual_hash = _sha256_file(public_dir / filename)
            if actual_hash != expected_hash:
                raise ValueError(f"Preserved workbook changed or is missing: {filename}")
