...
def test_facility_plans_six_rows_and_blank_separator():
    ...
    assert facility.item_rows[0] == ("6 chi phí cơ sở vật chất", 200)
...
def test_system_cost_plans_single_row():
    ...
    assert system_cost.item_rows == (("Một dòng chi phí tổng hợp", 210),)
...
def test_fixed_assets_plans_source_order_rows():
    ...
    assert fixed_assets.item_rows == (
        ("Khấu hao tài sản cố định", 207),
        ("Lãi vay tài sản cố định", 208),
    )
    assert fixed_assets.blank_row_after == 209
    assert "các dòng FORM cũ 38/42" in fixed_assets.notes
...
def test_birthday_and_nnn_plan_source_order_rows():
    ...
    assert planned["birthday"].item_rows == (("Chi phí sinh nhật", 214),)
    assert "các dòng FORM cũ 59/63" in planned["birthday"].notes
    assert planned["nnn_paperwork"].start_row == 218
    assert planned["nnn_paperwork"].item_rows == (("Hồ sơ NNN", 218),)
    assert "dòng FORM cũ 137" in planned["nnn_paperwork"].notes
