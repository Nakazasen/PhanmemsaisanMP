...
class TestNewHireAllocationIdentityDedupe(unittest.TestCase):
    ...
    def test_recruitment_health_uses_blank_canonical_row_58(self):
        conn = _mk_conn()
        builder = HubBuilder(conn, fiscal_year=2027)
        workbook = openpyxl.Workbook()
        ws = workbook.active
        ws.cell(58, 3, '=IFERROR(VLOOKUP($B58,A:H,2,0),"")')
        ws.cell(58, 18, "=SUM(F58:Q58)")
        self.assertEqual(builder._find_recruitment_health_row(ws), 58)
        workbook.close()
        conn.close()
    ...
    def test_recruitment_health_does_not_replace_unrelated_row_58(self):
        ...
        workbook = openpyxl.Workbook()
        ws = workbook.active
        ws.cell(58, 19, "Unrelated template content")
        with self.assertRaises(ExportIntegrityError):
            builder._find_recruitment_health_row(ws)
        workbook.close()
        conn.close()
